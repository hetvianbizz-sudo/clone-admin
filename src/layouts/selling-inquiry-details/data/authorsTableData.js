...
export default function AuthorsTableData() {
  ...
  const { companyId, catalogId } = useParams();
  ...
  const fetchUserList = async () => {
    try {
      ...
      const response = await axios.get(`${BASE_URL}/api/superadmin/catalogSellingInquiry/${catalogId}/${companyId}`, {
        headers: {
          Authorization: token,
        },
      });
      console.log(response.data)
      setCompanyDetails(response.data.data?.[0]?.selling_inquiry_details);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  return {
    ...
    rows: companyDetails && companyDetails?.map((category) => ({
        product: (
          <Author name={category?.product?.[0]?.name_of_chemical} email={category?.product?.[0]?.CAS_number} />
        ),
        buyer: (
          <MDTypography
            component="a"
            ...
            fontWeight="medium"
          >
            {category?.buyer_company?.[0]?.company_name}
          </MDTypography>
        ),
        type: (
          <MDTypography
        ...
        status: (
          <MDBox ml={-1}>
            <MDBadge
              badgeContent={category.status}
              color={category.status === "delivered" ? "success" : category.status === "pending" ? "secondary" : category.status === "deal done" ? "success" : category.status === "cancel" ? "error" : category.status === "rejected" ? "error" : "info"}
              variant="gradient"
              size="sm"
            />
          </MDBox>
        ...
        view: (
          <MDTypography
            ...
            color="text"
            fontWeight="medium"
            onClick={() => handleNavigate(category?.negotation?.[0]?.inquiryId)}
